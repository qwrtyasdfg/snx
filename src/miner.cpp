...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("snxMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("snxMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("snxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("snx-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running snxMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
